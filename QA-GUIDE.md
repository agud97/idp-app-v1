...
## Два способа создать окружение

| | Backstage UI | Git напрямую |
|---|---|---|
| Кому подходит | QA, ручное тестирование | Разработчики, CI/CD |
| Нужен git? | Нет — форма в браузере | Да |
| Нужен kubectl? | Нет | Нет |
| Результат | PR создаётся автоматически | Вы пушите сами |
| Скорость | Быстрее | Гибче |

**Рекомендуем начать с Backstage UI** — особенно если вы не работаете с git ежедневно.

---

## Способ 1: Backstage UI (рекомендуется)

### 1. Откройте Backstage

Попросите платформенную команду открыть port-forward:
```bash
kubectl --kubeconfig ~/proj/cross/kubeconfig_6005021 \
  port-forward svc/backstage 7007:7007 -n backstage
```

Затем откройте в браузере: **http://localhost:7007**

Логин не нужен — вход как гость автоматический.

### 2. Перейдите в шаблоны

Нажмите **Create...** в левом меню → выберите **"Create Test Environment"**.

### 3. Заполните форму

| Поле | Что вводить | Пример |
|---|---|---|
| **Environment Name** | Короткое имя, только строчные буквы и дефисы | `qa`, `staging`, `dev-alice` |
| **Enable Frontend** | Включить ли frontend | ✓ |
| **Frontend Replicas** | Количество реплик (по умолчанию 2) | `2` |
| **Enable Backend** | Включить ли backend + PostgreSQL | ✓ |
| **Backend DB Password** | Пароль к БД | `testpass` |
| **Enable Platform Services** | Включить ли gateway, auth, users, ... | ☐ |

### 4. Нажмите Next → Create

Backstage автоматически:
1. Генерирует `developer-apps/<имя>.yaml`
2. Создаёт ветку `backstage/env-<имя>` в репозитории
3. Открывает Pull Request

В конце вы увидите **ссылку на PR**.

### 5. Смержите PR

Откройте ссылку на PR → нажмите **Merge pull request**.

Через 1-2 минуты ArgoCD задеплоит окружение автоматически.
...
## Способ 2: Git напрямую (быстрый старт)

### 1. Склонируйте репозиторий (если ещё не склонировали)
...
**Q: Как лучше создать окружение — через Backstage или через git?**
A: Через Backstage удобнее: не нужен git, форма в браузере. Git подходит если нужна полная гибкость параметров (replicas, image, env, migration и т.д.) — шаблон Backstage покрывает базовые поля.

**Q: Backstage создаёт PR — нужно ли его мержить вручную?**
A: Да, всегда. Backstage не мержит PR автоматически — это намеренно, чтобы можно было проверить YAML перед деплоем. Откройте ссылку из Backstage и нажмите Merge.

**Q: Могу ли я изменить параметры окружения после создания через Backstage?**
A: Да. После мержа PR в репозитории появится файл `developer-apps/<имя>.yaml`. Отредактируйте его и запушьте — ArgoCD применит изменения.

**Q: Backstage недоступен, что делать?**
...
A: Измените `developer-apps/<имя>.yaml` и запушьте (или создайте новый PR). Изменения применятся автоматически.
